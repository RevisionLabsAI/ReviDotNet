...
internal class Infer
{
	...
	/// <param name="directRoute">
	/// When <c>true</c> and Forge is configured, bypasses Forge routing and calls the provider directly
	/// (useful for latency-sensitive requests). Usage is still reported back to Forge asynchronously.
	/// </param>
		...
		bool directRoute = false)
	{
		...
		{
			// When bypassing Forge routing, report usage back so Forge can still track it.
	...
	/// <param name="cancellationToken">Cancellation token for the operation.</param>
	/// <param name="directRoute">
	/// When <c>true</c> and Forge is configured, bypasses Forge routing and streams directly from the provider
	/// (useful for latency-sensitive requests). Usage is still reported back to Forge asynchronously.
	/// </param>
		...
		bool directRoute = false)
	{
		...
		// When bypassing Forge routing, accumulate output so we can report token usage back.
		bool reportToForge = directRoute && ForgeManager.Reporter is not null;
		System.Text.StringBuilder? outputAccumulator = reportToForge ? new System.Text.StringBuilder() : null;

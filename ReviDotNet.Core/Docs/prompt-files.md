...
| `gemini-safety-threshold` | string | `null` | **Gemini only.** Harm-block threshold applied to every harm category: `BLOCK_NONE`, `BLOCK_ONLY_HIGH`, `BLOCK_MEDIUM_AND_ABOVE`, `BLOCK_LOW_AND_ABOVE`, or `OFF`. Unset leaves Gemini's own defaults in place, which is right for ordinary prompts. Set it when the prompt's job is to *classify* content for safety: Gemini otherwise suppresses its own answer on exactly that input, returning null fields, and a caller that reads an unparseable verdict as "allowed" then fails open on the worst cases. Ignored by non-Gemini protocols. |
